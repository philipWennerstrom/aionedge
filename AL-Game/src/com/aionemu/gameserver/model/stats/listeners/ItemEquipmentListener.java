...
public class ItemEquipmentListener {
	...
	private static void onItemEquipment(Item item, CreatureGameStats<?> cgs) {
		...
		if (slot == ItemSlot.MAIN_HAND.getSlotIdMask() || slot == ItemSlot.SUB_HAND.getSlotIdMask()) {
			List<StatFunction> allModifiers = wrapModifiers(item, modifiers);
			if (rndBonuses != null) {
				allModifiers = StatFunction.mergeRandomBonuses(allModifiers, rndBonuses.getModifiers());
			}
			if (item.hasFusionedItem()) {
				...
				if (weaponStats != null) {
					...
					if (fusionedItemTemplate.getWeaponType() == WeaponType.ORB_2H || fusionedItemTemplate.getWeaponType() == WeaponType.BOOK_2H) {
			...
			List<StatFunction> allModifiers = modifiers;
			if (rndBonuses != null)
				allModifiers =	StatFunction.mergeRandomBonuses(allModifiers, rndBonuses.getModifiers());
			cgs.addEffect(item, allModifiers);
		}
