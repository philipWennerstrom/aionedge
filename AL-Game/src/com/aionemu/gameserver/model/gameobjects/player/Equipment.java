...
public class Equipment {
	...
	public Item unEquipItem(int itemUniqueId, long slot) {
		...
		synchronized (equipment) {
			...
			if (itemToUnequip.getItemTemplate().isArmor()
				&& itemToUnequip.getItemTemplate().getCategory() == ItemCategory.SHARD) {
	...
	public boolean hasDualWeaponEquipped(ItemSlot slot) {
		Item weaponSlot = equipment.get(slot.getSlotIdMask());
		if (weaponSlot != null) {
			if (weaponSlot.getItemTemplate() != null) {
				if (weaponSlot.getItemTemplate().getWeaponType() != null) {
					if (weaponSlot.getItemTemplate().getWeaponType().getRequiredSlots() == 1) {
						return true;	
					}
				}
			}
	...
	private boolean soulBindItem(final Player player, final Item item, final long slot) {
		...
		if (requested) {
			PacketSendUtility.sendPacket(player, new SM_QUESTION_WINDOW(
				SM_QUESTION_WINDOW.STR_SOUL_BOUND_ITEM_DO_YOU_WANT_SOUL_BOUND, 0, 0, new DescriptionId(item.getNameID())));
		}
		else {
