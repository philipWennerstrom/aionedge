...
public class Player extends Creature {
	...
	private boolean canPvP(Player enemy) {
		...
		if (!enemy.getRace().equals(getRace())) {
			...
			else if (worldId == 210040000 || worldId == 220040000) {
			   return (!this.isInDisablePvPZone() && !enemy.isInDisablePvPZone());
			}
			else {
				return true;
	...
	public boolean isAggroFrom(Npc npc) {
		...
		return (isAggroIconTo(npc) && (npc.getTribe().isGuard() || npc.getObjectTemplate().getAbyssNpcType() != AbyssNpcType.NONE
			|| npc.getLevel() + 10 > getLevel() || (npc.isInInstance() && InstanceService.isAggro(npc.getWorldId()))));
	}
	...
	public boolean isAggroIconTo(Npc npc) {
		...
		if (getAdminEnmity() == 1 || getAdminEnmity() == 3)
			return true;
		// Exception by Tribe and attackable type (if player can not attack, then NPC can not either)
		...
		if (npc.getNpcType().equals(NpcType.PEACE)) {
			return false;
		}

		if (npc.getNpcType().equals(NpcType.INVULNERABLE)) {
			return false;
		}

		if (npc.getNpcType() == NpcType.NON_ATTACKABLE) {
			if (npc.getWorldId() == 310010000 || npc.getWorldId() == 320010000)
				return false;
		}

		if (DataManager.TRIBE_RELATIONS_DATA.isSupportRelation(tribe, getTribe()))
			return false;

		switch (getTribe()) {
			case PC:
				if (tribe == TribeClass.XDRAKAN_DGUARD || race == Race.ASMODIANS || (tribe != null ? tribe.isDarkGuard() : true) ||
					race == Race.CONSTRUCT && tribe == TribeClass.PET_DARK)
					return true;
				return DataManager.TRIBE_RELATIONS_DATA.isAggressiveRelation(tribe, TribeClass.PC);
			case PC_DARK:
				if (tribe == TribeClass.XDRAKAN_LGUARD || race == Race.ELYOS || (tribe != null ? tribe.isLightGuard() : true) ||
					race == Race.CONSTRUCT && tribe == TribeClass.PET)
					return true;
				return DataManager.TRIBE_RELATIONS_DATA.isAggressiveRelation(tribe, TribeClass.PC_DARK);
				default:
					return DataManager.TRIBE_RELATIONS_DATA.isAggressiveRelation(tribe, getTribe());
		}
	}
