...
public class HousingBidService extends AbstractCronTask {
   ...
   public AuctionResult completeHouseSell(PlayerCommonData winner, House obtainedHouse) {
	  ...
	  if (winner.isOnline()) {
		 if (result == AuctionResult.WIN_BID) {
			...
			winner.getPlayer().setHousingStatus(HousingFlags.HOUSE_OWNER.getId());
	  ...
	  MailFormatter.sendHouseAuctionMail(obtainedHouse, winner, result, System.currentTimeMillis(), 0);
   ...
   public synchronized void placeBid(Player player, int entryIndex, long bidOffer) {
	  if ((player.getHousingStatus() & HousingFlags.BIDDING_ALLOWED.getId()) == 0) { //prevent this earlier (problem are house signs which allow bidding)
   ...
   /**
	 * Notify about new auction results or maintenance checks but only once (based
	 * on system mail checks and login time)
    */
