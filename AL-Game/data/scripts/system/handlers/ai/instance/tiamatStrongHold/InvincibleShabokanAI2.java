...
public class InvincibleShabokanAI2 extends AggressiveNpcAI2 {
   ...
   private void earthQuakeEvent() {
	  Npc invisible = getPosition().getWorldMapInstance().getNpc(283217);
	  SkillEngine.getInstance().getSkill(getOwner(), 20717, 55, getOwner()).useNoAnimationSkill();
	  if (invisible == null) {
		 spawn(283217, getOwner().getX(), getOwner().getY(), getOwner().getZ(), (byte) 0);
	  }
   }
   ...
   private void sinkEvent() {
	  ...
	  for (Player player : getKnownList().getKnownPlayers().values()) {
		 if (isInRange(player, 30)) {
			spawn(283218, player.getX(), player.getY(), player.getZ(), (byte) 0);
			spawn(283219, player.getX(), player.getY(), player.getZ(), (byte) 0);
		 }
	  }
   }
