...
public class UnstableYamenesAI2 extends AggressiveNpcAI2 {
				...
				if (isAlreadyDead()) {
					...
					ThreadPoolManager.getInstance().schedule(new Runnable() {
						...
						public void run() {
							...
							spawn(219974, boss.getX() + 10, boss.getY() - 10, boss.getZ(), (byte) 0);
							spawn(219974, boss.getX() - 10, boss.getY() + 10, boss.getZ(), (byte) 0);
							spawn(219974, boss.getX() + 10, boss.getY() + 10, boss.getZ(), (byte) 0);
	...
	private void spawnPortal() {
		Npc portalA = getPosition().getWorldMapInstance().getNpc(219955);
		Npc portalB = getPosition().getWorldMapInstance().getNpc(219967);
		Npc portalC = getPosition().getWorldMapInstance().getNpc(219968);
		if (portalA == null && portalB == null && portalC == null) {
			if (!top) {
				NpcShoutsService.getInstance().sendMsg(getOwner(), 1400637);
				spawn(219955, 288.10f, 741.95f, 216.81f, (byte) 3);
				spawn(219967, 375.05f, 750.67f, 216.82f, (byte) 59);
				spawn(219968, 341.33f, 699.38f, 216.86f, (byte) 59);
				top = true;
			}
			else {
				NpcShoutsService.getInstance().sendMsg(getOwner(), 1400637);
				spawn(219955, 303.69f, 736.35f, 198.7f, (byte) 0);
				spawn(219967, 335.19f, 708.92f, 198.9f, (byte) 35);
				spawn(219968, 360.23f, 741.07f, 198.7f, (byte) 0);
				top = false;
			}
		}
	}
